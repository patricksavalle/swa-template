...
- **Extract the Pure Core**: For any block of functionality, separate "decide"
  from "do." Extract deterministic, side-effect-free logic into pure functions
  that receive all inputs as values and return values describing the result.
- **I/O at the Edges**: Side effects — database, network, filesystem, time,
  randomness, logging, mutation, framework state, globals — belong only in the
  orchestration layer that gathers inputs, calls the pure core, and applies the
  returned effects.
- **Testability**: A pure core is unit-testable without mocks. If domain logic
  needs mocks, clocks, databases, files, network, globals, or framework state to
  test, purity has been violated.
